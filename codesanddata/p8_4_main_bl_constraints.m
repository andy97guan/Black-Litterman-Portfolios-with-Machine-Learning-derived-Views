...
ylabel('weights');
title('Constrained BL portfolio weights for \delta=0.01 (Near Kelly Investor)');
...
xlabel('Out of sample period');
ylabel('weights');
title('Constrained BL portfolio weights for \delta=2.24 (Average Investor) ');
legend('EFA','EEM','GLD','TLT','IYR','Location','bestoutside');
...
xlabel('Out of sample period');
ylabel('weights');
title('Constrained BL portfolio weights for \delta=6 (Risk Averse Investor) ');
legend('EFA','EEM','GLD','TLT','IYR','Location','bestoutside');
set(gcf, 'Color', 'w');
print(fig,'constraint_weights_sum.eps','-depsc2')
...
objValue =(delta/2)* x' * sig_bl * x -  mu_bl' * x ;
end

...
ylabel('weights');
title('Max Reward to CVaR portfolio weights for \delta=0.01 (Near Kelly Investor)');
...
xlabel('Out of sample period');
ylabel('weights');
title('Max Reward to CVaR portfolio weights for \delta=2.24 (Average Investor) ');
legend('EFA','EEM','GLD','TLT','IYR','Location','bestoutside');
...
xlabel('Out of sample period');
ylabel('weights');
title('Max Reward to CVaR portfolio weights for \delta=6 (Risk Averse Investor) ');
legend('EFA','EEM','GLD','TLT','IYR','Location','bestoutside');
set(gcf, 'Color', 'w');
print(fig,'CVaR_weights.eps','-depsc2')
...
VaR_ratio=mean_ret/sqrt(var_ret); %VaR ratio
sr_apa=VaR_ratio*sqrt(52);
...
alpha=0.05;
factor=normpdf(norminv(alpha))/(1-alpha);

objValue = (-1*x'*mu_bl) /(factor*sqrt(delta*x'*sig_bl*x)-x'*mu_bl) ;
end
